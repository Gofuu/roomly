...
| **Status** | Implemented; 109 automated tests; CI green on GitHub |
...
**Current (budget) deployment:** about US$10–15/month in ap-south-1. Every piece is defined as code:
- **Infrastructure:** CloudFormation, `infra/roomly-stack.yaml`.
- **Server:**
  - one Graviton **EC2 t4g.micro** runs `postgres`, a one-shot `migrate` container and the `api` container with **Docker Compose** (`deploy/docker-compose.prod.yml`);
  - an Elastic IP gives CloudFront a stable origin;
  - the security group admits port 80 **only from CloudFront's managed prefix list**, and there's no SSH (administration goes through SSM Session Manager).
- **CloudFront** is the single public origin, which keeps cookies same-site and WebSockets working:
  - `/*` is served from a private **S3** bucket through Origin Access Control;
  - a CloudFront Function rewrites client-side routes to `index.html`;
  - `/api/*` and `/socket.io/*` go to the server with caching disabled.
- **Secrets** live in **SSM Parameter Store** (`/roomly/*`). They're generated by the first deploy and written to a root-only `.env` on the host.
- **Delivery:**
  - CI builds an arm64 image on a native ARM runner and pushes it to GHCR;
  - the Deploy workflow assumes an AWS role through **GitHub OIDC** (no stored keys), syncs the SPA and host scripts to S3, runs `deploy.sh` through **SSM Run Command**, invalidates CloudFront and smoke-tests `/api/health`.
- **Upkeep:** systemd timers take a nightly `pg_dump` to S3 (7-day lifecycle) and reset the two demo companies, so public visitors always find a clean demo.

**Scale-up path** (no application changes):
- RDS PostgreSQL;
- ECS Fargate tasks behind an ALB (`TRUST_PROXY_HOPS=2`);
